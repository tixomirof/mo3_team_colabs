...
убирает личные данные и присваивает один из шести стеков:
...
и кладут в переменную окружения HH_ACCESS_TOKEN.
...
# Роль «Программист, разработчик» в справочнике hh.ru.
PROGRAMMER_ROLE_ID = "96"
DEFAULT_PER_PAGE = 50
DEFAULT_DELAY_SEC = 0.8
MAX_RETRIES = 4

# Не больше шести классов, как требуется в задании.
STACK_QUERIES = {
    "ASP.NET": "ASP.NET",
    "React": "React",
    "Python": "Python",
...
# Признаки стека: ищем в должности, навыках и описании опыта.
...
# Java не должен срабатывать на JavaScript.
...
def load_settings() -> None:
    """Подхватываем токен из локального .env, не из командной истории."""
...
class HeadHunterClient:
    """Клиент официального API hh.ru с паузой и повторными попытками."""
    ...
    def _access_message(response: requests.Response) -> str:
        ...
        return (
            ...
            "с услугой доступа к базе резюме. Логин и пароль парсеру не нужны. "
...
def join_skills(item: dict[str, Any]) -> str:
    ...
    # Сохраняем порядок, убираем повторы.
...
def join_experience(item: dict[str, Any]) -> str:
    ...
    for job in item.get("experience") or []:
        if not isinstance(job, dict):
            continue
        # Компанию не берём: для класса стека достаточно должности и обязанностей.
...
def prepare_for_java(text: str) -> str:
    # Иначе «JavaScript» даст ложное срабатывание класса Java.
...
def classify(record: dict[str, str], search_stack: str | None = None) -> tuple[str, int]:
    ...
    if best_score < 2:
        return "", 0
    # Смешанный стек не записываем: классы должны быть различимы.
...
def collect_resumes(
    ...
    """Ищем резюме отдельно по каждому стеку и затем перепроверяем класс."""
    ...
    classified: dict[str, list[dict[str, str]]] = {stack: [] for stack in STACK_QUERIES}
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--user-agent",
        help="Имя приложения и контакт, как требует hh.ru",
    )
...
def resolve_credentials(args: argparse.Namespace) -> tuple[str, str]:
    ...
    user_agent = (
        ...
        or "MO3ResumeParser/1.0 (student@example.com)"
    )
    if not token:
        raise AccessError(
            "Не задан токен. Создайте OAuth-токен на https://dev.hh.ru "
            "и запишите его в HH_ACCESS_TOKEN. Пароль аккаунта не нужен."
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print("Классы:", ", ".join(STACK_QUERIES))
        return 0
